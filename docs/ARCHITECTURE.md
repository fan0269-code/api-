...
RelayHub 当前版本是可交付的前后端一体化网站，用于演示 API 中转站的公开官网、开发者控制台、API Key 管理、模型列表、渠道管理、用量统计、调用日志、账单余额和接入文档。
...
3. 前端调用 `POST /api/auth/login` 获取控制台会话 token，随后并行拉取账户、Key、模型、渠道、用量、调用日志、账单和文档示例。
...
9. 中转调用成功后，后端写入用量点、调用日志、账单记录、Key 已用额度和最近使用时间。
...
- 调用日志只返回最近 100 条，后端持久化时最多保留 200 条，避免演示环境 JSON 数据无限增长。
...
- 生产级密钥加密存储尚未接入第三方服务；当前 `/v1/chat/completions` 已提供兼容接口、鉴权、渠道校验、上游转发、SSE 流式透传、用量记录、调用日志和 mock 兜底。
